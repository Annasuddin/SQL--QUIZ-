...
ORDER BY so.TotalAmount DESC) AS OrderRank
...
so.OrderDate,
sod.Quantity,
LAG(sod.Quantity) OVER (PARTITION BY p.ProductID
ORDER BY so.OrderDate) AS PrevQuantity,
LEAD(sod.Quantity) OVER (PARTITION BY p.ProductID 
ORDER BY so.OrderDate) AS NextQuantity
...
AS
BEGIN
SELECT p.ManufacturerID AS SupplierID,SUM(so.TotalAmount) AS TotalSalesAmount
FROM Product AS p
...
EXEC sp_GetSupplierSales @SupplierID = 5;
